...
def main() -> None:
    try:
        ...
        crop = center_crop(gray, crop_h=400, crop_w=400)

        crop_3d = crop[..., np.newaxis]  # (400, 400, 1)
        print(f"The shape of image is: {crop_3d.shape}")
        print(crop_3d)
        ...
        plt.figure()
        plt.imshow(transposed, cmap="gray", origin="upper")
        plt.xlabel("X (px)")
        plt.ylabel("Y (px)")
        plt.title("Transposed 400×400")
        plt.tight_layout()
        plt.show()
